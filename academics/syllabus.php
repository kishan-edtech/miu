            ...
            <ol class="breadcrumb d-flex flex-wrap justify-content-between align-self-start">
              ...
              <div>
                  <?php
                    ...
                    { ?>
                        <button class="btn btn-link" aria-label="" title="" data-toggle="tooltip" data-original-title="Add Subject" onclick="add('subjects','md');"> <i class="ti ti-copy-plus add_btn_form  add_btn_form"></i></button>
                <button class="btn btn-link" aria-label="" title="" data-toggle="tooltip" data-original-title="Upload" onclick="add('subjects/upload', 'lg');"> <i class="uil uil-export"></i></button>
          ...
          <div class="col-md-6">
            <div class="form-group form-group-default required">
              ...
              <select class="full-width" style="border: transparent;" id="course" onchange="getSemester(this.value); removeTable()">
                ...
                $sub_courses = $conn->query("SELECT CONCAT(Courses.Short_Name, ' (', Sub_Courses.Name, ')') AS Sub_Course, Sub_Courses.ID FROM Sub_Courses LEFT JOIN Courses ON Sub_Courses.Course_ID = Courses.ID WHERE Sub_Courses.University_ID = " . $_SESSION['university_id'] . " $condition ORDER BY Courses.Name ASC");
    ...
    <script type="text/javascript">
      function getTable() {
        ...
        if (course_id.length > 0 && semester.length > 0) {
          $.ajax({
            url: BASE_URL + '/app/subjects/syllabus-without-lms?course_id=' + course_id + '&semester=' + semester,
            type: 'GET',
            success: function(data) {
              $('#subjects').html(data);
            }
