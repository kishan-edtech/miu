        ...
        if ($late_fees->num_rows > 0) {
            while ($late_fee = $late_fees->fetch_assoc()) {
                ...
                if (in_array($student['Admission_Session_ID'], $admission_session) && $date >= $late_fee['Start_Date'] && $student['Admission_Type_ID'] == $late_fee['admission_type']) {
                    // die;
                    $student_fee = ['Late Fine' => $late_fee['Fee']];
                    // print_r($student_fee );
...
if (! function_exists('sendMail')) {
    function sendMail($sender, $receivers, $mail, $cc = [], $bcc = [], $replyTo = [], $scheduleAt = null)
    {
        $data = [];
        if (! empty($replyTo)) {
        ...
        if (! empty($mail['Attachments'])) {
            ...
            foreach ($attachments as $attachment) {
                $files[] = ["url" => "https://admission-portal.com" . $attachment['path'], "name" => $attachment['name'], "type" => $attachment['type']];
            }
...
if (! function_exists('numberToWordFunc')) {
    ...
    function numberToWordFunc($number)
    {
        ...
        $wordsTeen = ['Ten', 'Eleven', 'Twelve', 'Thirteen', 'Fourteen', 'Fifteen', 'Sixteen', 'Seventeen', 'Eighteen', 'Nineteen'];
        $wordsTens = ['', '', 'Twenty', 'Thirty', 'Forty', 'Fifty', 'Sixty', 'Seventy', 'Eighty', 'Ninety'];
