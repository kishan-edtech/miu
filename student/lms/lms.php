...
<div class="page-container ">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row all_subject">
                       ...
                       $sub_counts = $conn->query("SELECT Syllabi.Name ,ID  as subject_id FROM Syllabi WHERE Course_ID='" . $_SESSION['Course_ID'] . "' AND Sub_Course_ID='" . $_SESSION['Sub_Course_ID'] . "' AND Semester = '" . $_SESSION['Duration'] . "' AND University_ID ='" . $_SESSION['university_id'] . "'");
                            ...
                            while ($subResArr = $sub_counts->fetch_assoc()) {
                                ...
                                <div class="col-md-3">
                                    <div class="card info-box p-0">
                                        <a
                                            href="/ams/student/lms/subjects?id=<?= $subResArr['subject_id'] ?>&type=1&duration=<?= $duration ?>">
                                        ...
                                        <div class="card-footer">
                                            <div class="row justify-content-between align-items-center">
                                                ...
                                                <?php
                                                $ebook_query = $conn->query("SELECT id FROM e_books WHERE subject_id = '" . $subResArr['subject_id'] . "' AND sub_course_id='" . $_SESSION['Sub_Course_ID'] . "' and Status = 1");
                                                $ebook_count = ($ebook_query->num_rows > 0) ? $ebook_query->num_rows : 0;
                              
                                                $note_query = $conn->query("SELECT id FROM notes WHERE subject_id = '" . $subResArr['subject_id'] . "' AND Course_ID='" . $_SESSION['Sub_Course_ID'] . "' and Status = 1");
                                                $note_count = ($note_query->num_rows > 0) ? $note_query->num_rows : 0;

                                                $video_query = $conn->query("SELECT id FROM video_lectures WHERE subject_id = '" . $subResArr['subject_id'] . "' AND sub_course_id='" . $_SESSION['Sub_Course_ID'] . "' and Status = 1");
                                                $video_count = ($video_query->num_rows > 0) ? $video_query->num_rows : 0;
                                                ?>
                                                <div class="col-md-4 text-center">
                                                    <a
                                                        href="/ams/student/lms/subjects?id=<?= $subResArr['subject_id'] ?>&type=1&duration=<?= $duration ?>"><i
                                                            class="ti-book mr-2"></i><span><?= $ebook_count; ?></span></a>
                                                </div>
                                                <div class="col-md-4 text-center">
                                                    <a
                                                        href="/ams/student/lms/study-material?subject=<?= $subResArr['subject_id'] ?>&type=2&duration=<?= $duration ?>"><i
                                                            class="ti- ti-video-clapper mr-2"></i><span><?= $video_count; ?></span></a>
                                                </div>
                                                <div class="col-md-4 text-center">
                                                    <a
                                                        href="/ams/student/lms/subjects?id=<?= $subResArr['subject_id'] ?>&type=4&duration=<?= $duration ?>"><i
                                                            class=" ti-clipboard mr-2"></i><span><?= $note_count ?></span></a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
    ...
    <script type="text/javascript">
        ...
        function getSemester(id, val = null) {
            ...
            $.ajax({
                url: BASE_URL + '/app/subjects/semester?id=' + id + "&onload=" + val,
                type: 'GET',
    ...
    <script type="text/javascript">
        function getTable() {
            ...
            if (course_id.length > 0 && semester.length > 0) {
                $.ajax({
                    url: BASE_URL + '/app/subjects/syllabus?course_id=' + course_id + '&semester=' + semester + '&lms=lms',
                    type: 'GET',
                    success: function (data) {
    ...
    <script>
        function E_bookList(id, unit_id, sub_id,) {
            ...
            $.ajax({
                url: BASE_URL + '/app/e-books/students/show-list',
                type: 'POST',
                data: {
                    "id": id,
                    "sub_id": sub_id,
    ...
    <script>
        function addAssessment(id, unit_id, sub_id) {
            $.ajax({
                url: BASE_URL + '/app/e-books/assessments/create?unit_id=' + unit_id + '&syllabus_id=' + sub_id + '&ebook_id=' + id,
                type: 'GET',
                success: function (data) {
    ...
    <script>
        function assessmentList(assessment_id, ebook_id, unit_id, sub_id) {
            $.ajax({
                url: BASE_URL + '/app/e-books/students/assessments/show-list',
                type: 'POST',
                data: {
    ...
    <script>
        function startAssessment(assessment_id, e_book_id, unit_id, suject_id) {
            $('.modal').modal('hide');
            $.ajax({
                url: BASE_URL + '/app/e-books/students/assessments/store',
                type: 'POST',
                data: {
