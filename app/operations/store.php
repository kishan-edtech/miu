<?php
 ...
 $vertical = mysqli_real_escape_string($conn, $_POST['vertical']??"");
    ...
    $add = $conn->query("INSERT INTO `Users`(`Name`, `Code`, `Email`, `Mobile`, `Password`, `Photo`, `Role`, `Designation`, `Created_By`,`vertical`) VALUES ('$name', '$code', '$email', '$contact', AES_ENCRYPT('$contact','60ZpqkOnqn0UQQ2MYTlJ'), '$filename', 'Operations', 'Operations', ".$_SESSION['ID'].",$vertical)");
    if($add){
      echo json_encode(['status'=>200, 'message'=>'Operations added successfully!']);
    }else{
