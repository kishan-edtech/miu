<?php
  if(isset($_POST['name']) && isset($_POST['university_id'])){
    ...
    $check = $conn->query("SELECT ID FROM Departments WHERE (Name like '$name') AND University_ID = $university_id");
    if($check->num_rows>0){
      echo json_encode(['status'=>400, 'message'=>$name.' already exists!']);
      exit();
    }
    
    $add = $conn->query("INSERT INTO `Departments`(`Name`, `University_ID`) VALUES ('$name', $university_id)");
    if($add){
