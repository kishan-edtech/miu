...
if (isset($_GET['id'])) {
  ...
  $id = intval($_GET['id']);
  $user = $conn->query("SELECT Name, Code, Email, Mobile, vertical FROM Users WHERE ID = $id");
...
<div class="modal-header clearfix text-left">
  ...
  <h5 class="text-black font-weight-bold">Edit <span class="semi-bold">University Head</span></h5>
</div>
<form role="form" id="form-edit-university-heads" action="/ams/app/university-heads/update" method="POST" enctype="multipart/form-data">
  <div class="modal-body">
    <div class="row">
        <div class="col-md-12 pr-0">
          <div class="form-group form-group-default required">
            <label>Vertical</label>
            <select class="full-width" style="border: transparent;" name="vertical" id="vertical">
    ...
    <div class="row">
      ...
      <div class="col-md-6">
        <div class="form-group form-group-default required">
          <label>Contact</label>
          <input type="tel" name="contact" class="form-control" placeholder="ex: 9998777655" minlength="10" maxlength="10" onkeypress="return isNumberKey(event)" value="<?= $user['Mobile'] ?>" required>
        </div>
      </div>
    </div>
...
<script>
  $(function() {
    $('#form-edit-university-heads').validate({
      rules: {
        name: {
          required: true
        },
  ...
  $("#form-edit-university-heads").on("submit", function(e) {
    if ($('#form-edit-university-heads').valid()) {
