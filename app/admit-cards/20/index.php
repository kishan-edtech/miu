<?php
  ...
  if(isset($_GET['id'])){
    ...
    $student = $conn->query("SELECT Students.ID, Students.Sub_Course_ID, Students.First_Name, Students.Middle_Name, Students.Last_Name, Students.Father_Name, Students.Enrollment_No, Students.Duration, Students.Contact, Students.Unique_ID, Students.DOB, Students.Address, Sub_Courses.Short_Name, Admission_Sessions.Name as Session FROM Students LEFT JOIN Sub_Courses ON Students.Sub_Course_ID = Sub_Courses.ID LEFT JOIN Admission_Sessions ON Students.Admission_Session_ID = Admission_Sessions.ID WHERE Students.ID = $id AND Students.University_ID = ".$_SESSION['university_id']."");
    ...
    $y = 115;
    $counter = 1;
    $syllabi = $conn->query("SELECT Syllabi.*, Date_Sheets.Exam_Date FROM Syllabi LEFT JOIN Date_Sheets ON Syllabi.ID = Date_Sheets.Syllabus_ID WHERE Sub_Course_ID = ".$student['Sub_Course_ID']." AND Semester = ".$student['Duration']." AND Paper_Type = 'Theory' ORDER BY Code ASC");
