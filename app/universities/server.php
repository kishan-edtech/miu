...
## Fetch records
$result_record = "SELECT `ID`, `Short_Name`, `Vertical`, `Logo`, `Is_B2C`, `Course_Allotment`, `Sharing`, `Has_LMS`, `Has_Unique_Center`, `Center_Suffix`, `Has_Unique_StudentID`, `ID_Suffix`, `Max_Character`, `Status` Status FROM Universities WHERE ID IS NOT NULL $searchQuery $orderby LIMIT ".$row.",".$rowperpage;
...
while ($row = mysqli_fetch_assoc($empRecords)) {
    ...
    $data[] = array( 
      ...
      "Logo" => $row["Logo"],
      "Is_B2C" => $row["Is_B2C"],
      "Course_Allotment" => $row["Course_Allotment"],
      "Sharing" => $row["Sharing"],
      "Has_LMS" => $row["Has_LMS"],
