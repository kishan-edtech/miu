...
if (isset($_POST['inserted_id'])) {
    ...
    // Get old academic data before update
    $old_query = $conn->query("SELECT Type, `Year`, Subject, `Board/Institute`, Marks_Obtained, Max_Marks, Total_Marks FROM Student_Academics
    WHERE Student_ID = $inserted_id");

    $old_data = [];

    while ($row = $old_query->fetch_assoc()) {
        $old_data[$row['Type']] = $row;
    }

    $step              = $conn->query("SELECT Step, Sub_Courses.Eligibility, Admission_Type_ID FROM Students LEFT JOIN Sub_Courses ON Students.Sub_Course_ID = Sub_Courses.ID WHERE Students.ID = $inserted_id");
    $step              = mysqli_fetch_array($step);
    $eligibility       = $step['Eligibility'];
    $admission_type_id = $step['Admission_Type_ID'];
    $step              = $step['Step'];
    ...
    $eligibility = $eligibility[$admission_type_id]['required'];
    ...
    // Get new academic data after update
    $new_query = $conn->query("
    SELECT Type, `Year`, Subject, `Board/Institute`, Marks_Obtained, Max_Marks, Total_Marks
...
");
    ...
    foreach ($new_data as $type => $fields) {

        if (! isset($old_data[$type])) {
            $changes_new[$type] = $fields;
            continue;
        }

        foreach ($fields as $key => $value) {

            if ($old_data[$type][$key] != $value) {

                $changes_old[$type][$key] = $old_data[$type][$key];
                $changes_new[$type][$key] = $value;
            }
        }
    }
    ...
    if ($update) {
        ...
        if (! empty($changes_new)) {
            ...
            addLog($conn,$_SESSION['university_id'], $_SESSION['ID'], 'update', 'Student_Academics', $inserted_id, 'Student academics updated (Step 3)', json_encode($changes_old), json_encode($changes_new));
        }
