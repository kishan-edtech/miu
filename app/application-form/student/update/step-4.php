...
if (isset($_FILES)) {
  ...
  if (isset($_FILES["other_certificate"]["tmp_name"]) && $_FILES["other_certificate"]['tmp_name'] != '' && count(array_filter($_FILES["other_certificate"]['tmp_name'])) > 0) {
    foreach ($_FILES["other_certificate"]["tmp_name"] as $key => $tmp_name) {
      ...
      if (in_array($other_certificate_extension, $allowed_file_extensions)) {
        ...
        echo json_encode(['status' => 302, 'message' => 'Other Certificate should be image!']);
        exit();
      }
    }
  ...
  if ($update) {
    ...
    $_SESSION['printFromId'] = base64_encode('W1Ebt1IhGN3ZOLplom9I' . $inserted_id);
    $_SESSION['Step'] = 5;

    // Send Last Step Mail
    $leadStatusId = 0;
    $leadId = $conn->query("SELECT Lead_Status_ID FROM Students WHERE Lead_Status_ID IS NOT NULL AND ID = " . $inserted_id);
    if ($leadId->num_rows > 0) {
      $leadId = $leadId->fetch_assoc();
      $leadStatusId = $leadId['Lead_Status_ID'];

      // Send Fee Receipt
      $mail = $conn->query("SELECT Subject as subject, Template as body, Attachments FROM Email_Templates WHERE University_ID = " . $_SESSION['university_id'] . " AND Name = 'Application Form Completed'");
      if ($mail->num_rows > 0) {
        $mail = $mail->fetch_assoc();

        // Sender
        $university = $conn->query("SELECT Name as name, Email as email FROM Universities WHERE ID = " . $_SESSION['university_id']);
    ...
    echo json_encode(['status' => 200, 'message' => 'Step 4 details saved successfully!']);
  } else {
