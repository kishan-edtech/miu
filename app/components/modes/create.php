<?php if(isset($_GET['university_id'])){ ?>
  ...
  <div class="modal-header clearfix text-left">
    ...
    <h6 class="text-black font-weight-bold">Add <span class="semi-bold">Mode</span></h6>
  </div>
  <form role="form" id="form-add-modes" action="/ams/app/components/modes/store" method="POST">
    <div class="modal-body">
      <div class="row">
        <div class="col-md-12">
          <div class="form-group form-group-default required">
            <label>Name</label>
            <input type="text" name="name" class="form-control" placeholder="ex: Sem">
          </div>
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    $("#form-add-modes").on("submit", function(e){
      if($('#form-add-modes').valid()){
