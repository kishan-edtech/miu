<?php
  if(isset($_GET['university_id']) && isset($_GET['mode']) && isset($_GET['durations']) && isset($_GET['id']) && isset($_GET['scheme'])){
    ...
    if(empty($university_id) || empty($mode) || empty($durations) || empty($id) || empty($scheme)){
      exit();
    }
    ...
    // Fee Structures
    $fee_structures = $conn->query("SELECT Fee_Structure FROM Schemes WHERE ID = $scheme");
    $fee_structures = $fee_structures->fetch_assoc();
    $fee_structures = json_decode($fee_structures['Fee_Structure'], true);

    $vocational_query = "";
    $check_for_vocational = $conn->query("SELECT ID FROM Universities WHERE ID = $university_id AND Course_Allotment	= 1");
    if($check_for_vocational->num_rows>0){
      $vocational_query = " AND Name NOT LIKE 'Course%'";
    }

    $fee_structures = $conn->query("SELECT ID, Name, Fee_Applicable_ID FROM Fee_Structures WHERE Status = 1 AND Is_Constant = 1 AND ID IN (".implode(",", $fee_structures).") AND University_ID = $university_id $vocational_query ORDER BY Name ASC");
