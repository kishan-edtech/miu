<?php
  if(isset($_GET['university_id']) && isset($_GET['mode']) && isset($_GET['durations']) && isset($_GET['scheme'])){
    ...
    if(empty($university_id) || empty($mode) || empty($durations) || empty($scheme)){
      exit();
    }
    ...
    $vocational_query = "";
    $check_for_vocational = $conn->query("SELECT ID FROM Universities WHERE ID = $university_id AND Sharing	= 0");
    if($check_for_vocational->num_rows>0){
      $vocational_query = " AND Sharing = 0";
    }
