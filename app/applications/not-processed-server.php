...
if ($searchValue != '') {
  if (strcasecmp($searchValue, 'completed') == 0) {
    ...
    $searchQuery = " AND (RIGHT(CONCAT('000000', Students.ID), 6) like '%" . $searchValue . "%' OR Students.ID like '%" . $searchValue . "%' OR Students.Unique_ID like '%" . $searchValue . "%' OR Students.First_Name like '%" . $searchValue . "%' OR Students.Middle_Name like '%" . $searchValue . "%' OR Students.Last_Name like '%" . $searchValue . "%' OR Admission_Sessions.Name like '%" . $searchValue . "%' OR Admission_Types.Name like '%" . $searchValue . "%' OR Students.Step like '%" . $searchValue . "%' OR Students.Father_Name like '%" . $searchValue . "%' OR Students.Email like '%" . $searchValue . "%' OR Students.Contact like '%" . $searchValue . "%' OR Sub_Courses.Short_Name like '%" . $searchValue . "%')";
  }
}
...
$filterByProcessDate = "";
if (isset($_SESSION['filterByProcessDate'])) {
  $filterByProcessDate = $_SESSION['filterByProcessDate'];
}
$filterByStatus = "";
...
if($_SESSION['Role']=="University Head"){
    $uniQuery = " AND Students.University_ID IN ( ".$_SESSION['all_university_ids'] . ")";
    
}else{
    $uniQuery = " AND Students.University_ID = ".$_SESSION['university_id'];
}

$searchQuery .= $filterByDepartment.$filterBySubCourse.$filterByProcessDate .$filterByDate.$filterByStatus.$filterQueryUser.$uniQuery. $filterByVerticalType.$filterByDuration;
// 
## Total number of records without filtering
$all_count = $conn->query("SELECT COUNT(Students.ID) as allcount FROM Students LEFT JOIN Admission_Sessions ON Students.Admission_Session_ID = Admission_Sessions.ID WHERE 1=1 AND Process_By_Center IS NULL AND Step = 4 $role_query $step_query $session_query");
$records = mysqli_fetch_assoc($all_count);
$totalRecords = $records['allcount'];
// echo('<pre>');print_r("SELECT COUNT(Students.ID) as filtered FROM Students LEFT JOIN Admission_Sessions ON Students.Admission_Session_ID=Admission_Sessions.ID LEFT JOIN Admission_Types ON Students.Admission_Type_ID=Admission_Types.ID LEFT JOIN Sub_Courses ON Students.Sub_Course_ID=Sub_Courses.ID LEFT JOIN Users ON Students.Added_For=Users.ID WHERE 1=1 AND Process_By_Center IS NULL AND Step = 4 $searchQuery $role_query $step_query $session_query");die;
## Total number of record with filtering
$filter_count = $conn->query("SELECT COUNT(Students.ID) as filtered FROM Students LEFT JOIN Admission_Sessions ON Students.Admission_Session_ID=Admission_Sessions.ID LEFT JOIN Admission_Types ON Students.Admission_Type_ID=Admission_Types.ID LEFT JOIN Sub_Courses ON Students.Sub_Course_ID=Sub_Courses.ID LEFT JOIN Users ON Students.Added_For=Users.ID WHERE 1=1 AND Process_By_Center IS NULL AND Step = 4 $searchQuery $role_query $step_query $session_query");
$records = mysqli_fetch_assoc($filter_count);
$totalRecordwithFilter = $records['filtered'];

## Fetch records
$result_record = "SELECT Student_Pendencies.ID as Pendency, UPPER(DATE_FORMAT(Students.DOB, '%d%b%Y')) as DOB, Students.Status, Students.ID, Students.Added_For, Students.Unique_ID, CONCAT(Students.First_Name, ' ', Students.Middle_Name, ' ', Students.Last_Name) as First_Name, Students.Father_Name, Students.Enrollment_No, Students.OA_Number, Students.Duration, Students.Step, Students.Process_By_Center, Students.Payment_Received, Students.Document_Verified, Students.Processed_To_University, Admission_Sessions.`Name` as Adm_Session, Admission_Types.`Name` as Adm_Type, CONCAT(Courses.Short_Name, ' (', Sub_Courses.Name, ')') as Short_Name, Student_Documents.`Location`, Students.ID_Card, Students.Admit_Card, Students.Exam FROM Students LEFT JOIN Student_Pendencies ON Students.ID = Student_Pendencies.Student_ID AND Student_Pendencies.Status = 0 LEFT JOIN Admission_Sessions ON Students.Admission_Session_ID = Admission_Sessions.ID LEFT JOIN Admission_Types ON Students.Admission_Type_ID = Admission_Types.ID LEFT JOIN Courses ON Students.Course_ID = Courses.ID LEFT JOIN Sub_Courses ON Students.Sub_Course_ID = Sub_Courses.ID LEFT JOIN Student_Documents ON Students.ID = Student_Documents.Student_ID AND Student_Documents.`Type` = 'Photo' WHERE 1=1 AND Process_By_Center IS NULL AND Step = 4 $searchQuery $role_query $step_query $session_query $orderby LIMIT " . $row . "," . $rowperpage;
...
while ($row = mysqli_fetch_assoc($empRecords)) {
  ...
  if ($_SESSION['Role'] == 'Center') {
    ...
    $user = $conn->query("SELECT ID, Code, Name FROM Users WHERE ID = " . $row['Added_For'] . " AND Role != 'Sub-Center'");
    if ($user->num_rows == 0) {
  ...
  if (!empty($user)) {
    ...
    if($_SESSION['university_id']==41){
        ...
        $admissionSession = $row['Adm_Session'];
        ...
        if ($duration > 0) {
            ...
            // Wapas usi format mein return karo (e.g., Apr-2026)
            $exam_session = $date->format('M-Y');
        } else {
            $exam_session = $Adm_Session; // Agar koi match nahi mila
        }
