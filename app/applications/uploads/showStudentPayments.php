...
WHERE usp.ID = $uniStuPayId";
          ...
          while ($row = mysqli_fetch_assoc($result)) {
              ...
              foreach ($students as $student) {
                  ...
                  // Fetch student name
                  $stuQuery  = "SELECT Unique_ID,First_Name ,Middle_Name,Last_Name,Status,Enrollment_No,Processed_To_University,Payment_Received FROM Students WHERE ID = '$student'";
                  $stuResult = mysqli_query($conn, $stuQuery);
                  $stuData   = mysqli_fetch_assoc($stuResult);

                  $studentName = trim($stuData['First_Name'] . ' ' .$stuData['Middle_Name'] . ' ' .$stuData['Last_Name']);
                  
                  $rowStatus = "Not Enrolled";
                  if (!empty(trim($stuData['Enrollment_No']))) {
                    $rowStatus = "Enrolled";
                  } 
                //   else {
                //     $processedToUniversity = ($stuData['Processed_To_University'] == 1) ? "Not Processed" : "Processed to University";

                //     $paymentVerified = ($stuData['Payment_Received'] == 1) ? "Not Verified" : $processedToUniversity;
                
                //     $rowStatus =  $paymentVerified;
                // }
              ?>
        <tr>
          <td><?php echo $row['Payment_ID']; ?></td>
          <td><?php echo trim($studentName).' ('.$stuData['Unique_ID'] .') '; ?></td>
