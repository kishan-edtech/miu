...
if (isset($_POST['course_id']) && isset($_POST['semester']) && isset($_POST['start_date']) && isset($_POST['end_date'])) {
  ...
  if ($checkForRescheduled->num_rows > 0) {
    $isRescheduled = 1;
    $date_sheets = $conn->query("SELECT Reschedule_Date_Sheets.ID, Students.Unique_ID, CONCAT(Students.First_Name, ' ', Students.Middle_Name, ' ', Students.Last_Name) as First_Name, Exam_Sessions.`Name` AS examSession,CONCAT(Courses.Short_Name,' (',Sub_Courses.`Name`,')')AS course,Syllabi.Code,Syllabi.`Name`,Syllabi.Semester,Reschedule_Date_Sheets.Exam_Date,Reschedule_Date_Sheets.Start_Time,Reschedule_Date_Sheets.End_Time,(SELECT COUNT(ID)FROM Students WHERE Exam=1 AND Students.Sub_Course_ID=Syllabi.Sub_Course_ID AND Students.Duration=Syllabi.Semester $addedFor)AS studentCount,(SELECT COUNT(ID)FROM MCQs WHERE Reschedule_Date_Sheet_ID=Reschedule_Date_Sheets.ID)AS questionCount FROM Reschedule_Date_Sheets LEFT JOIN Exam_Sessions ON Reschedule_Date_Sheets.Exam_Session_ID=Exam_Sessions.ID LEFT JOIN Syllabi ON Reschedule_Date_Sheets.Syllabus_ID=Syllabi.ID LEFT JOIN Sub_Courses ON Syllabi.Sub_Course_ID=Sub_Courses.ID LEFT JOIN Courses ON Sub_Courses.Course_ID=Courses.ID LEFT JOIN Students ON Reschedule_Date_Sheets.Student_ID = Students.ID WHERE Reschedule_Date_Sheets.University_ID= " . $_SESSION['university_id'] . " $condition ORDER BY Exam_Date ASC");
  } else {
    $date_sheets = $conn->query("SELECT Date_Sheets.ID, Students.Unique_ID, CONCAT(Students.First_Name, ' ', Students.Middle_Name, ' ', Students.Last_Name) as First_Name, Exam_Sessions.`Name` as examSession, CONCAT(Courses.Short_Name,' (',Sub_Courses.`Name`,')')as course, Syllabi.Code, Syllabi.`Name`, Syllabi.Semester, Date_Sheets.Exam_Date, Date_Sheets.Start_Time, Date_Sheets.End_Time FROM Date_Sheets LEFT JOIN Exam_Sessions ON Date_Sheets.Exam_Session_ID = Exam_Sessions.ID LEFT JOIN Syllabi ON Date_Sheets.Syllabus_ID = Syllabi.ID LEFT JOIN Sub_Courses ON Syllabi.Sub_Course_ID = Sub_Courses.ID LEFT JOIN Courses ON Sub_Courses.Course_ID = Courses.ID LEFT JOIN Students ON Syllabi.Sub_Course_ID = Students.Sub_Course_ID AND Syllabi.Semester = Students.Duration AND Students.Exam = 1 WHERE Date_Sheets.University_ID = " . $_SESSION['university_id'] . " AND Students.Unique_ID IS NOT NULL $condition ORDER BY Students.Unique_ID ASC, Exam_Date ASC");
  }
...
?>
    <div class="table-responsive">
      <table id="exam-data-table" class="table table-striped nowrap">
        <thead>
          <tr>
            <th>#</th>
            <th>Exam Session</th>
        ...
        <tbody>
          ...
          ?>
            <tr>
              ...
              <td><?= $date_sheet['Unique_ID'] ?></td>
              <td><?= $date_sheet['First_Name'] ?></td>
