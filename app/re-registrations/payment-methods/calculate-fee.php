...
foreach ($ids as $id) {
  ...
  if ($student->num_rows > 0) {
    ...
    $fee = $conn->query("SELECT Fee, Fee_Without_Sharing as Center_Fee FROM Student_Ledgers WHERE Student_ID = $id AND Duration = $duration AND Type = 1 AND Source IS NULL");
    if ($fee->num_rows == 0) {
      continue;
    }
    ...
    $fee = $fee->fetch_assoc();
    $totalStudentFee = 0;
    foreach(json_decode($fee['Fee']) as $schemeId => $amount){
        $totalStudentFee = $totalStudentFee + $amount;
    }
    $totalCenterFee = 0;
    foreach(json_decode($fee['Center_Fee']) as $schemeId => $amount){
        $totalCenterFee = $totalCenterFee + $amount;
    }
    
    $totalFee[$id] = $_SESSION['Role'] == 'Sub-Center' ? $totalStudentFee : ($totalCenterFee!=0 ? $totalCenterFee : $totalStudentFee);
    // print_r($totalFee);die;
    // Late Fee
    $startDate = date("Y-m-d");
    $lateFees = $conn->query("SELECT ID, End_Date, Fee, Exception, Admission_Session FROM Late_Fees WHERE University_ID = " . $student['University_ID'] . " AND Start_Date <= '$startDate' AND Status = 1 AND For_Students = 'Re-Reg' AND IsLateFee = 1 ORDER BY ID DESC");
