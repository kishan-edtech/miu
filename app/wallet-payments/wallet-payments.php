...
if ($_SESSION['Role'] == "Sub-Center") {
  $ids = $_SESSION['ID'];
  $getInvoiceId = $conn->query("SELECT Invoice_No FROM Wallet_Invoices LEFT JOIN Students ON Wallet_Invoices.Student_ID = Students.ID WHERE Added_For = $ids GROUP BY Invoice_No");
  $invoice_no = [];
  while ($invoiceIds = $getInvoiceId->fetch_array(MYSQLI_ASSOC)) {
    $invoice_no[] = "'" . $invoiceIds['Invoice_No'] . "'";
  }
  if (!empty($invoice_no)) {
    $invoiceids = implode(", ", $invoice_no);
    $role_query = " AND Wallet_Payments.Transaction_ID IN ($invoiceids)";
  }
}
