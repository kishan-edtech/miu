<?php 
  if(isset($_GET['id'])){
    ...
    session_start();
    $id = intval($_GET['id']);
    $user = $conn->query("SELECT vertical, Name, Short_Name, Contact_Name, Code, Email, Mobile, Alternate_Mobile, Address, Pincode, State, City, District FROM Users WHERE ID = $id");
    $user = mysqli_fetch_assoc($user);

    $userUniversity = $conn->query("SELECT University_ID,User_ID  FROM University_User WHERE User_ID = $id");
    $userUniversity = mysqli_fetch_assoc($userUniversity);
  }
?>
...
<div class="modal-header clearfix text-left">
  ...
  <h5 class="font-weight-bold text-black">Edit <span class="semi-bold"></span>Center</h5>
</div>
  ...
  <div class="row">
        <div class="col-md-12 pr-0">
          <div class="form-group form-group-default required">
            ...
            <select class="full-width" style="border: transparent;" name="university_id" id="university_id">
              ...
              <option value="<?php echo $university['ID']; ?>" <?php if(isset($userUniversity['University_ID']) && $userUniversity['University_ID'] == $university['ID']) { echo "selected";}?>><?php echo $university['Short_Name']." (".$university['Vertical'].")"; ?></option>
              <?php } } ?>
            </select>
          </div>
        </div>
    ...
    <div class="row">
      <div class="col-md-6">
          <div class="form-group form-group-default">
        <label>Photo*</label>
        <input type="file" name="photo" accept="image/png, image/jpg, image/jpeg, image/svg"></div>
      </div>
    </div> 
  </div>
