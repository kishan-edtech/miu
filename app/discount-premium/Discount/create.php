...
if (isset($_GET['id'])) {
  ...
  while ($ledger = $ledgerQuery->fetch_assoc()) {
      ...
      // CASE 1 → University_ID != 41
      if ($ledger['University_ID'] != 41) {

          // TYPE = 1 → normal duration
          if ($ledger['Type'] == 1) {
         ...
         $courseFeeNumeric = (-1 * $balance);
              ...
              // TYPE != 1 → duration + 1 logic
              $studentDuration = $student['Duration'] + 1;

              $ledgerQuery2 = $conn->query("
                  SELECT *
                  FROM Student_Ledgers
                  WHERE Student_ID = '{$id}'
                    AND Status = 1
              ...
              while ($ledger2 = $ledgerQuery2->fetch_assoc()) {

                  $fees = json_decode($ledger2['Fee'], true);
                  if (!is_array($fees)) continue;

                  foreach ($fees as $value) {
                      $debit  = ($ledger2['Type'] == 1) ? $value : 0;
                      $credit = ($ledger2['Type'] == 2) ? $value : 0;
                      $balance = ($balance + $credit) - $debit;
                  }
              }
...
<form role="form" id="form-discount" action="/ams/app/discount-premium/Discount/store" method="POST">
    <div class="modal-body">
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <p class="text-black">Current Discount: <b><?= $discount ?></b></p>
            </div>
        </div>
    ...
    <div class="modal-footer flex justify-content-between">
        ...
        <!--<div class="m-t-10 sm-m-t-10">-->
            <?php if ($discount > 0) { ?>
                <!--<button type="button" onclick="deleteDiscount('<?= $id ?>')" class="btn btn-danger btn-cons btn-animated from-left">-->
                <!--    <span>Delete</span>-->
...
$("#form-discount").on("submit", function(e) {
    e.preventDefault();
  var discount = <?= floatval($discount) ?>;
var courseFee = <?= floatval($courseFeeNumeric) ?>; // numeric fee only

// Validation
if (discount >= courseFee) {
    notification('danger', 'Discount cannot be greater than or equal to Total Fees');
    return false;
}
...
function deleteDiscount(id) {
    Swal.fire({
        title: 'Are you sure?',
        text: "Discount will be set to 0",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "/ams/app/discount-premium/Discount/destroy?id=" + id,
                type: 'DELETE',
                dataType: 'json',
                success: function(data) {
                    if (data.status == 200) {
