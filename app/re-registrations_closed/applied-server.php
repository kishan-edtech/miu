...
$all_count = $conn->query("SELECT COUNT(Re_Registrations.ID) as allcount FROM Re_Registrations LEFT JOIN Students ON Re_Registrations.Student_ID = Students.ID WHERE Re_Registrations.University_ID = " . $_SESSION['university_id'] . " AND Re_Registrations.Exam_Session_ID = " . $_SESSION['active_rr_session_id'] . " $role_query");
$records = mysqli_fetch_assoc($all_count);
$totalRecords = $records['allcount'];

## Total number of record with filtering
$filter_count = $conn->query("SELECT COUNT(Re_Registrations.ID) as filtered FROM Re_Registrations LEFT JOIN Students ON Re_Registrations.Student_ID = Students.ID LEFT JOIN Courses ON Students.Course_ID = Courses.ID LEFT JOIN Sub_Courses ON Students.Sub_Course_ID = Sub_Courses.ID LEFT JOIN Admission_Sessions ON Students.Admission_Session_ID = Admission_Sessions.ID WHERE Re_Registrations.University_ID = " . $_SESSION['university_id'] . " AND Re_Registrations.Exam_Session_ID = " . $_SESSION['active_rr_session_id'] . " $searchQuery $role_query");
$records = mysqli_fetch_assoc($filter_count);
$totalRecordwithFilter = $records['filtered'];

## Fetch records
$result_record = "SELECT Re_Registrations.ID, Students.Duration, CONCAT(Students.First_Name, ' ', Students.Middle_Name, ' ', Students.Last_Name) as First_Name, Students.Enrollment_No, Students.Unique_ID, Admission_Sessions.Name as Admission_Session_ID, CONCAT(Courses.Short_Name, ' (', Sub_Courses.Name,')') as Course_ID, Students.Added_For FROM Re_Registrations LEFT JOIN Students ON Re_Registrations.Student_ID = Students.ID LEFT JOIN Courses ON Students.Course_ID = Courses.ID LEFT JOIN Sub_Courses ON Students.Sub_Course_ID = Sub_Courses.ID LEFT JOIN Admission_Sessions ON Students.Admission_Session_ID = Admission_Sessions.ID WHERE Re_Registrations.University_ID = " . $_SESSION['university_id'] . " AND Re_Registrations.Exam_Session_ID = " . $_SESSION['active_rr_session_id'] . " $role_query $searchQuery $role_query $orderby LIMIT " . $row . "," . $rowperpage;
