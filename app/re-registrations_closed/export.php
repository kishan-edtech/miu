...
foreach ($examSessions as $value) {
  ...
  if ($value['table'] == 'Fresh') {
    ...
    $students = $conn->query("SELECT ID FROM Students WHERE University_ID = " . $_SESSION['university_id'] . " $condition");
    ...
    $students = $conn->query("SELECT Student_ID as ID FROM Re_Registrations LEFT JOIN Students ON Re_Registrations.Student_ID = Students.ID WHERE Re_Registrations.University_ID = " . $_SESSION['university_id'] . " $condition");
  }
