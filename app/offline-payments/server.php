...
$filter_count = $conn->query("SELECT COUNT(Payments.ID) as filtered FROM Payments LEFT JOIN Universities ON Payments.University_ID = Universities.ID WHERE Type = 1 $university_query $searchQuery $role_query");
$records = mysqli_fetch_assoc($filter_count);
$totalRecordwithFilter = $records['filtered'];

## Fetch records
$result_record = "SELECT Payments.ID, Payments.Transaction_Date, Payments.Transaction_ID, Payments.Gateway_ID, Payments.Amount, Payments.File, Payments.Payment_Mode, Payments.Bank, Payments.Added_For, Payments.Added_By, Payments.Status, CONCAT(Universities.Short_Name, ' (', Universities.Vertical, ')') as University FROM Payments LEFT JOIN Universities ON Payments.University_ID = Universities.ID WHERE Type = 1 $university_query $searchQuery $role_query $orderby LIMIT " . $row . "," . $rowperpage;
...
while ($row = mysqli_fetch_assoc($empRecords)) {
  ...
  // Added_For
  $user = $conn->query("SELECT ID, Code, Name FROM Users WHERE ID = " . $row['Added_By'] . " AND (Role = 'Center' OR Role='Sub-Center')");
  if ($user->num_rows == 0 &&  !empty($row['Added_For'])) {
