...
if (isset($_GET['id'])) {
  ...
  $eligibility = $conn->query("SELECT Eligibility,Students.Admission_Type_ID FROM Students LEFT JOIN Sub_Courses ON Students.Sub_Course_ID = Sub_Courses.ID WHERE Students.ID = $id");
  $eligibility = $eligibility->fetch_assoc();
  $admissionTypeID = $eligibility['Admission_Type_ID'];
  $eligibility = !empty($eligibility['Eligibility']) ? json_decode($eligibility['Eligibility'], true) : [];
  
//   print_r(  "SELECT Eligibility FROM Students LEFT JOIN Sub_Courses ON Students.Sub_Course_ID = Sub_Courses.ID WHERE Students.ID = $id" );die;
?>
  <!-- Modal -->
  ...
  <form role="form" id="report-pendency-form" action="/ams/app/pendencies/store" method="POST" enctype="multipart/form-data">
    <div class="modal-body">
      <?php
      $documents = array('Photo', 'Student Signature', 'Parent Signature', 'Aadhar Front', 'Aadhar Back', 'Affidavit', 'Migration', 'Other Certificate');
      ...
      foreach($marksheets as $key=>$value){
        if(in_array($key, $eligibility[$admissionTypeID]['required'])){
          $documents[] = $value;
        }
      }
