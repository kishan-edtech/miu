...
if (isset($_POST['id'])) {
  ...
  if (isset($_FILES["aadhar_front"]["tmp_name"]) && $_FILES["aadhar_front"]['tmp_name'] != '' && count(array_filter($_FILES["aadhar_front"]['tmp_name'])) > 0) {
    foreach ($_FILES["aadhar_front"]["tmp_name"] as $key => $tmp_name) {
      $aadhar = mysqli_real_escape_string($conn, $_FILES["aadhar_front"]["name"][$key]);
      $tmp_name = $_FILES["aadhar_front"]["tmp_name"][$key];
      $aadhar_extension = pathinfo($aadhar, PATHINFO_EXTENSION);
      $aadhar_name = $inserted_id . "_Aadhar_Front" . $key . "." . $aadhar_extension;
      if (in_array($aadhar_extension, $allowed_file_extensions)) {
        ...
        if (move_uploaded_file($tmp_name, $aadhar_folder . $aadhar_name)) {
          $aadhars_front[] = str_replace('../..', '', $aadhar_folder) . $aadhar_name;
        } else {
  ...
  if (isset($_FILES["aadhar_back"]["tmp_name"]) && $_FILES["aadhar_back"]['tmp_name'] != '' && count(array_filter($_FILES["aadhar_back"]['tmp_name'])) > 0) {
    foreach ($_FILES["aadhar_back"]["tmp_name"] as $key => $tmp_name) {
      $aadhar = mysqli_real_escape_string($conn, $_FILES["aadhar_back"]["name"][$key]);
      $tmp_name = $_FILES["aadhar_back"]["tmp_name"][$key];
      $aadhar_extension = pathinfo($aadhar, PATHINFO_EXTENSION);
      $aadhar_name = $inserted_id . "_Aadhar_Back_" . $key . "." . $aadhar_extension;
      if (in_array($aadhar_extension, $allowed_file_extensions)) {
        if (file_exists($aadhar_folder . $aadhar_name)) {
          unlink($aadhar_folder . $aadhar_name);
        }
        if (move_uploaded_file($tmp_name, $aadhar_folder . $aadhar_name)) {
          $aadhars_back[] = str_replace('../..', '', $aadhar_folder) . $aadhar_name;
        } else {
          echo json_encode(['status' => 503, 'message' => 'Unable to upload Aadhar!']);
          exit();
        }
