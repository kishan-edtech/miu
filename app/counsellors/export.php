<?php
  ...
  ## Fetch records
  $result_record = "SELECT `ID`, `Name`, `Email`, `Mobile`, `Code`, `Status`, `Photo`, CAST(AES_DECRYPT(Password, '60ZpqkOnqn0UQQ2MYTlJ') AS CHAR(50)) password FROM Users LEFT JOIN University_User ON Users.ID = University_User.User_ID WHERE Role = 'Counsellor' $university_query $searchQuery GROUP BY Users.ID $orderby";
  ...
  while ($row = mysqli_fetch_assoc($empRecords)) {
    ...
    $centers = $conn->query("SELECT Code FROM Alloted_Center_To_Counsellor LEFT JOIN University_User ON Alloted_Center_To_Counsellor.Counsellor_ID = University_User.User_ID WHERE Alloted_Center_To_Counsellor.Counsellor_ID = ".$row['ID']." $university_query");
    ...
    $row['Mobile'] = maskPhone($row['Mobile'] ?? '');
    $data[] = array( 
      $row['Name'],
      $row['Email'],
      $row['Mobile'],
  ...
  $xlsx = SimpleXLSXGen::fromArray( $data )->downloadAs('Counsellors.xlsx');
