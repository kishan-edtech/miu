                          ...
                          <div class="card-body" style="min-height: 150px !important">
                            <div class="row m-t-20">
                              <div class="d-flex justify-content-center col-md-12 cursor-pointer">
                                <img src="<?php echo '/ams/'.$university['Logo'] ?>" style="max-width:100% !important" height="100px">
                              </div>
                            </div>
                          </div>
                        </div>
...
<script type="text/javascript">
  ...
  function getComponents(id){
    $.ajax({
      url:'/ams/app/components/main?id='+id,
      type: "GET",
...
function addComponents(url, modal, university_id){
    $.ajax({
      url: '/ams/app/components/'+url+'/create?university_id='+university_id,
      type: 'GET',
      success: function(data){
  ...
  function editComponents(url, id, modal){
    $.ajax({
      url: '/ams/app/components/'+url+'/edit?id='+id,
      type: 'GET',
      success: function(data){
        $('#'+modal+'-modal-content').html(data);
        $('#'+modal+'modal').modal('show');
